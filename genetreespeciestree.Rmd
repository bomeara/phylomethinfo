...
plot(gene.tree)
```
*using windows, skipping out on this exercise, even though Id much rather prefer to simulate!*
...
```
*cannot compare, cannot infer gene tree.* 
...
plot(cophylo(species.tree, gene.tree, cbind(sort(species.tree$tip.label), sort(gene.tree$tip.label)))) #cannot complete. 
```
**function sim.coaltree.phylo() in phybase**
...
plot(cophylo(species.tree2, gene.tree2, cbind(sort(species.tree2$tip.label), sort(gene.tree2$tip.label)))) #cannot complete. 
```
*cannot simulate, function in phybase* 
